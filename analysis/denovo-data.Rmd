...
mutation=c("A->C", "A->G", "A->T", "C->G", "C->T", "C->A")
```

```{r, echo=F}
chr20_AtoC=load("D:\\ResearchWork\\StatisticalGenetics\\Mutation-Rate-project\\parameter_estimate\\para.est.chr20.AtoC.RData")
chr20_para_est=matrix(nrow=length(method), ncol=length(cova))
chr20_para_est[1,]=para.est$glm.before.cate[2:6]
chr20_para_est[2,]=para.est$glm.cate[2:6]
chr20_para_est[3,]=para.est$optim.before.cate
chr20_para_est[4,]=para.est$optim.cate
rownames(chr20_para_est)=method
colnames(chr20_para_est)=cova

plot(chr20_para_est[1,], ylim=c(min(chr20_para_est)-1, max(chr20_para_est)+1), type="o", xaxt='n', xlab="Covariate features", ylab=expression(paste(widehat(beta))), col=1, main="mutation A->C on chr20", cex=0.8)
lines(chr20_para_est[2,], col=2, type="o")
lines(chr20_para_est[3,], col=3, type="o")
lines(chr20_para_est[4,], col=4, type="o")
legend(4, -4, method, col=c(1,2,3,4), lty=c(1,1,1,1))
axis.labels <-cova
