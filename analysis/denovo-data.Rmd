...
* Continuous features are converted  into 50 binary discrete variables.  
...
### Whole genome-7 features 


```{r, echo=F}
mut.type=c("A_to_C", "A_to_G", "A_to_T", "C_to_A", "C_to_G", "C_to_T")
cova=c("DHS", "CpGisland", "Lamin", "GC", "RT", "RR", "Exon")
```
...
barcenters=barplot(cova.across.mut, beside=T,ylab=expression(paste(beta)),col=c("red"), ylim=c(min(conf.inter, cova.across.mut), max(conf.inter, cova.across.mut)),  main=cova[cova.index],  cex.names=1, args.legend = list(x ='topright', bty='n', inset=c(-0.07,0), cex=0.8), xpd = FALSE)
...
* Fit 7 covariate features simultaneously.
