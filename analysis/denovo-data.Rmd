...
### whole genome NO categorization-3 features (at most)
...
* GLM (use R function GLM)
  ...
  (1) The response is the ** total number of mutations ** in the category and the each feature is averaged across all widnows within the category (denoted as M1 (method 1))
  (11)  The response is the ** averaged  number of mutations (mutation rate) ** in the category and the each feature is averaged across all widnows within the category (denoted as M2 (method 2))
  (2) intercept is included and the same across all categories
  (3) confidence interval could be obtained as well

* use R function optim to fit the model (M3 method 3)
...
DHS=matrix(nrow=3, ncol=6)
DHS[1,]=optim.para$DHS
DHS[2,]=glm.total.mut.para$DHS
DHS[3,]=glm.ave.mut.para$DHS
colnames(DHS)=optim.para[,1]

barplot(DHS, beside=T, col=c("red", "darkblue", "green"), legend=c("M3", "M1", "M2"), ylim=c(min(DHS), max(DHS)+0.8), main="DHS by different methods")

CpG=matrix(nrow=3, ncol=6)
CpG[1,]=optim.para$CpGIsland
CpG[2,]=glm.total.mut.para$CpGIsland
CpG[3,]=glm.ave.mut.para$CpGIsland
colnames(DHS)=optim.para[,1]

barplot(CpG, beside=T, col=c("red", "darkblue", "green"), legend=c("M3", "M1", "M2"), ylim=c(min(CpG), max(CpG)+0.8), main="CpG island by different methods")


lamin=matrix(nrow=3, ncol=6)
lamin[1,]=optim.para$Lamin
lamin[2,]=glm.total.mut.para$Lamin
lamin[3,]=glm.ave.mut.para$Lamin
colnames(lamin)=optim.para[,1]

barplot(lamin, beside=T, col=c("red", "darkblue", "green"), legend=c("M3", "M1", "M2"), ylim=c(min(lamin), max(lamin)+0.8), main="Lamin by different methods")


GC=matrix(nrow=3, ncol=6)
GC[1,]=optim.para$GC
GC[2,]=glm.total.mut.para$GC
GC[3,]=glm.ave.mut.para$GC
colnames(GC)=optim.para[,1]

barplot(GC, beside=T, col=c("red", "darkblue", "green"), legend=c("M3", "M1", "M2"), ylim=c(min(GC), max(GC)+0.8), main="GC content by  different methods")

Reptime=matrix(nrow=3, ncol=6)
Reptime[1,]=optim.para$RepTime
Reptime[2,]=glm.total.mut.para$RepTime
Reptime[3,]=glm.ave.mut.para$RepTime
colnames(Reptime)=optim.para[,1]

barplot(Reptime, beside=T, col=c("red", "darkblue", "green"), legend=c("M3", "M1", "M2"), ylim=c(min(Reptime), max(Reptime)+0.8), main="Replication timing by  different methods")


Inter=matrix(nrow=2, ncol=6)
Inter[1,]=glm.total.mut.para$Intercept
Inter[2,]=glm.ave.mut.para$Intercept
colnames(Inter)=optim.para[,1]

barplot(Inter, beside=T, col=c( "darkblue", "green"), legend=c( "M1", "M2"), ylim=c(min(Inter), max(Inter)+8), main="Intercept by  different methods")
