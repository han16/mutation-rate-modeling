...
rownames(count)=c(0,1,2, ">2")
colnames(count)=cutoff
barplot(count, col=c("red", "blue", "green", "yellow"), beside=T, legend=c(0,1,2, ">2"), ylim=c(0,2), xlab="Standardized residual cutoff", ylab="Freq", main=mut.type[i],  args.legend=list(title="No.consecutive windows"))
} # end of i
```
...
* P values: In the whole genome, randomly pick the same amount of hotspot windows, do a enrichment test between hotspot windows and random windows under each scenario by one-side poisson test.    
...
hist(closest.neigh.mut.type[[j]][[i]], freq=F, breaks=50, main=paste("cutoff of ", cutoff[i], "; mean of ", round(ave),  sep=""), xlab="")
