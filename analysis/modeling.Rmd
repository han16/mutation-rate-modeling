...
For the Poission regression model where $\beta$ is fixed, the log predictive density is $l(\tilde{y}|\widehat{\beta})=\sum_jY_jlog(\widehat{\mu}_j)-\sum_j\widehat{\mu}_j$, where $\mu_j$ is the predicted mutation rate using the parameter estimated in the training set. 
