...
Let $c$ index category. $Y_c$ is the total number of mutations, $\mu_c$ is the baseline mutation rate, $X_c$ is the feature covariates. Then $Y_c \sim Pois(\mu_ce^{X_c\beta})$ with the probability mass function $P(Y_c|\beta)=\frac{\lambda_c^{Y_c}e^{-\lambda_c}}{Y_c!}$, where $\lambda_c=\mu_ce^{X_c\beta}$. 
...
\begin{aligned}
 ...
 &=\sum_c[Y_clog(\lambda_c)-\lambda_c-log(Y_c!) ]\\
 &=\sum_c\{Y_c[log(\mu_c)+X_c\beta]-\mu_ce^{X_c\beta}-log(Y_c!) \} \\
 &=\sum_c\{Y_c(X_c\beta)-\mu_ce^{X_c\beta} \}+constant
\end{aligned}
$$


Let $f(\beta)=\sum_c\{Y_c(X_c\beta)-\mu_ce^{X_c\beta} \}$ be the objective function to be optimized over $\beta$. 
$\frac{\partial f(\beta)}{\partial \beta_i}=\sum_c (Y_c(X_{ic})-\mu_cX_{ic}e^{X_c\beta})$, $i=1, \cdots, P$. 
