...
{
sum.lkhd=cate.matrix[,1]*cate.matrix[,2:(1+feature_number)]*beta-baseline.mutation*exp(cate.matrix[,2:(1+feature_number)]*beta)
 return(sum(sum.lkhd))
}

cate.grr=function(beta)
{
 partial.deriv=numeric(feature_number)
 for (i in 1:feature_number)
  {
    sum.fea=cate.matrix[,1]*cate.matrix[,(1+i)]-baseline.mutation*cate.matrix[,(1+i)]*exp(cate.matrix[,2:(1+feature_number)]*beta)
  partial.deriv[i]=sum(sum.fea)
  }
...
fr=function(beta)
{
sum.lkhd=discrete.feature[,1]*discrete.feature[,2:(1+feature_number)]*beta-baseline.mutation.before.cate*exp(discrete.feature[,2:(1+feature_number)]*beta)
 return(sum(sum.lkhd))
}
...
grr=function(beta)
{
 ...
 for (i in 1:feature_number)
  {
    sum.fea=discrete.feature[,1]*discrete.feature[,(1+i)]-baseline.mutation.before.cate*discrete.feature[,(1+i)]*exp(discrete.feature[,2:(1+feature_number)]*beta)
  partial.deriv[i]=sum(sum.fea)
  }
...
para.initial=rep(0.1, feature_number)
...
beta1.est[3,]=para.est.optim.before.cate$par
beta1.est[4,]=para.est.optim.cate$par
...
* M11 and M12 are GLM without and with categorization; M21,  M22 are optim without and with categorization. 
