...
* split Model 1 (M1) into two submodels, M1(CpG):3-mer+features for pooled 3 CpG mutation types and M1(nonCpG): 3-mer+features for pooled 6 nonCpG mutation types  
...
y.limit=max(m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs[,1:2])
x.limit=y.limit
outlier=which.max((m1.CpG.exp.obs+m1.nonCpG.exp.obs)[,2])
outlier.index=c(outlier, outlier+nrow(m0.exp.obs), outlier+3*nrow(m0.exp.obs), outlier+4*nrow(m0.exp.obs))
ggplot(m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs[-c(outlier.index),], aes(x=exp, y=obs, color=model)) +
  geom_point()+
  # xlim(c(0, x.limit))+ylim(c(0, y.limit))+
   geom_abline(intercept = 0, slope = 1)+
...
#######
#MSE:Mo: 
obs.exp=m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs%>%filter(model=="M0")
cat("M0 has MSE of", sum((obs.exp$obs-obs.exp$exp)^2)/nrow(obs.exp), "\n")

obs.exp=m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs%>%filter(model=="M1")
cat("M1 has MSE of", sum((obs.exp$obs-obs.exp$exp)^2)/nrow(obs.exp), "\n")

obs.exp=m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs%>%filter(model=="M0(CpG+nonCpG)")
cat("M0(CpG+nonCpG) has MSE of", sum((obs.exp$obs-obs.exp$exp)^2)/nrow(obs.exp), "\n")

obs.exp=m0.m1.m0CpGnonCpG.m1.CpCnonCpG.exp.obs%>%filter(model=="M1(CpG+nonCpG)")
cat("M1(CpG+nonCpG) has MSE of", sum((obs.exp$obs-obs.exp$exp)^2)/nrow(obs.exp), "\n")
```
