...
### Data by mutation type 


* Step 1: go to folder: `/project2/xinhe/Shengtong/mutation_rate/mutation.rate/Simons_WGS/controls`
* step 2: find mutation type by folder name, `C_to* has CpG and non CpG subfolder` 
* step 3: after go to folder/subfolder, search be file by `ls *and*ExpObsVar.bed` 

Note (1) the columns of bed file are chr, start, end, observed rate, expected rate, lower bound, upper bound, stand_diff, pvalue. (2) I combined odd window and even windows together, so you will see half odd windows and another half even windows. 
...
* Model 2 with variable selection (M2(variable selection)): among 14 covariates features, only significant ones are kept to fit the model.  Significant variables are defined as top 4 variables in terms of the ratio of coefficient estimate over their CI. 
...
* M2(vS-top4); among 14 covariates, pick the top 4 covariates with highest ratio of parameter estimate over CI

* M2 (VS-zscore), among 14 covariate features, choose the ones with |z score|>2 to refit the model 

* the higher predicted probability, the better, M1>M1(CpG)+M1(nonCpG)>M0>M2>M2(variable selection). 

* The way that calculates the predicted probability for 3 CpG combined sample and 6 nonCpG combined sample is the same as in M1 by treating the subset of 3 CpG combined sample as the whole sample. 
...
m2.post.pred.variable.selection=as_tibble(read.table(paste(path, "post.pred.for.model.with.variable.selection.txt", sep="/"), header=T))
m2.post.pred.variable.selection.zscore=as_tibble(read.table(paste(path, "post.pred.for.model.with.variable.selection.with.zscore.txt", sep="/"), header=T))

pp2.variable.selection=m2.post.pred.variable.selection%>%select(post.pred)%>%summarise(sum=sum(post.pred))%>%pull()
pp2.variable.selection.zscore=m2.post.pred.variable.selection.zscore%>%select(post.pred)%>%summarise(sum=sum(post.pred))%>%pull()
...
total.post.pred=tibble(model=c("M0", "M1", "M1(CpG+nonCpG)", "M2", "M2(VS-top4)", "M2(VS-zscore)"), post.pred=c(pp0, pp1, (pp1.CpG+pp1.nonCpG), pp2, pp2.variable.selection, pp2.variable.selection.zscore))
...
## Parameter estimate comparison between M1 & M2
...
## Parameter estimate from combined sample 
...
### Model M1 (all 9 mutation types combined)
...
ggplot(parameter.estimate.model1, aes(x=cova, y=estimate, fill=window)) + 
   ...
   theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=10))+
theme(text = element_text(size=10))+
   ylim(c(-0.65, 0.4))+ # use same scales to make comparison easier 
   ylab(expression(paste(hat(beta))))+xlab("covariate features")+
    geom_errorbar(aes(ymin=lower, ymax=upper),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9))+
    ggtitle("Parameter estimate in M1 (9 mutation types combined)")+
   theme(plot.title = element_text(hjust = 0.5, size=15)) # center the title
```

### Model M1(3 CpG mutation types combined)

```{r, echo=F, message=F, warning=F}
path="C:\\Shengtong\\Research\\mutation-rate\\parameter_estimate\\Simons\\100bpwindow\\"
cova=c("DHS", "CpGisland", "Lamin", "GC", "RT", "RR", "Exon", "H3K27ac", "H3K27me3", "H3K36me3", "H3K4me1","H3K4me3","H3K9ac", "H3K9me3")

### this is parameter estimate in M1(CpG) 
parameter.estimate.odd.window=loadRData(paste(path, "table.ASCWGS_20180504.WGS1902_hg19_controls_SNV_remove_recurrent_", "mutation.3CpG.combined", ".100bpwindow.odd.line.train.model.with.standardize.covariate.RData", sep=""))
parameter.estimate.even.window=loadRData(paste(path, "table.ASCWGS_20180504.WGS1902_hg19_controls_SNV_remove_recurrent_", "mutation.3CpG.combined", ".100bpwindow.even.line.train.model.with.standardize.covariate.RData", sep=""))

parameter.estimate.even.window.3CpG.combined=tibble(estimate=parameter.estimate.even.window$para.est, lower=parameter.estimate.even.window$para.conf[,1], upper=parameter.estimate.even.window$para.conf[,2])

parameter.estimate.odd.window.3CpG.combined=tibble(estimate=parameter.estimate.odd.window$para.est, lower=parameter.estimate.odd.window$para.conf[,1], upper=parameter.estimate.odd.window$para.conf[,2])

parameter.estimate.3CpG.combined=rbind(parameter.estimate.even.window.3CpG.combined[2:15,], parameter.estimate.odd.window.3CpG.combined[2:15,]) %>% add_column(window=c(rep("even",14), rep("odd", 14)), cova=rep(cova,2))


# Use 95% confidence intervals 
ggplot(parameter.estimate.3CpG.combined, aes(x=cova, y=estimate, fill=window)) + 
    geom_bar(position=position_dodge(), stat="identity") +
   theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=10))+
theme(text = element_text(size=10))+
   ylim(c(-0.65, 0.4))+
   ylab(expression(paste(hat(beta))))+xlab("covariate features")+
    geom_errorbar(aes(ymin=lower, ymax=upper),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9))+
    ggtitle("Parameter estimate in M1(3CpG combined)")+
   theme(plot.title = element_text(hjust = 0.5, size=15)) # center the title

```

### Model M1(6 nonCpG mutation types combined)
...
### this is parameter estimate in M1(CpG) 
parameter.estimate.odd.window=loadRData(paste(path, "table.ASCWGS_20180504.WGS1902_hg19_controls_SNV_remove_recurrent_", "mutation.6nonCpG.combined", ".100bpwindow.odd.line.train.model.with.standardize.covariate.RData", sep=""))
parameter.estimate.even.window=loadRData(paste(path, "table.ASCWGS_20180504.WGS1902_hg19_controls_SNV_remove_recurrent_", "mutation.6nonCpG.combined", ".100bpwindow.even.line.train.model.with.standardize.covariate.RData", sep=""))

parameter.estimate.even.window.6nonCpG.combined=tibble(estimate=parameter.estimate.even.window$para.est, lower=parameter.estimate.even.window$para.conf[,1], upper=parameter.estimate.even.window$para.conf[,2])

parameter.estimate.odd.window.6nonCpG.combined=tibble(estimate=parameter.estimate.odd.window$para.est, lower=parameter.estimate.odd.window$para.conf[,1], upper=parameter.estimate.odd.window$para.conf[,2])

parameter.estimate.6nonCpG.combined=rbind(parameter.estimate.even.window.6nonCpG.combined[2:15,], parameter.estimate.odd.window.6nonCpG.combined[2:15,]) %>% add_column(window=c(rep("even",14), rep("odd", 14)), cova=rep(cova,2))


# Use 95% confidence intervals 
ggplot(parameter.estimate.6nonCpG.combined, aes(x=cova, y=estimate, fill=window)) + 
    geom_bar(position=position_dodge(), stat="identity") +
   theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=10))+
theme(text = element_text(size=10))+
   ylim(c(-0.65, 0.4))+
   ylab(expression(paste(hat(beta))))+xlab("covariate features")+
    geom_errorbar(aes(ymin=lower, ymax=upper),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9))+
    ggtitle("Parameter estimate in M1(6nonCpG combined)")+
   theme(plot.title = element_text(hjust = 0.5, size=15)) # center the title
...
```{r, echo=F, message=F, warning=F}
window.type="even"
parameter.estimate.even.window.diff.model.combine=rbind(parameter.estimate.model1%>%filter(window==window.type), parameter.estimate.3CpG.combined %>% filter(window==window.type), parameter.estimate.6nonCpG.combined %>% filter(window==window.type)) %>% add_column(model=rep(c("9 mutation combined", "3 CpG combined", "6 nonCpG combined"), each=length(cova)))

# Use 95% confidence intervals 
ggplot(parameter.estimate.even.window.diff.model.combine, aes(x=cova, y=estimate, fill=model)) + 
    geom_bar(position=position_dodge(), stat="identity") +
   theme(axis.text.x = element_text(angle = 60, vjust = 0.5, size=10))+
theme(text = element_text(size=10))+
   ylim(c(-0.65, 0.4))+
   ylab(expression(paste(hat(beta))))+xlab("covariate features")+
    geom_errorbar(aes(ymin=lower, ymax=upper),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9))+
    ggtitle(paste(expression("Parameter estimate in even windows")))+
   theme(plot.title = element_text(hjust = 0.5, size=15)) # center the title
...
theme(text = element_text(size=10))+
   ...
   ylab(expression(paste(hat(beta))))+xlab("covariate features")+
    geom_errorbar(aes(ymin=lower, ymax=upper),
                  width=.2,                    # Width of the error bars
                  position=position_dodge(.9))+
    ggtitle(expression(paste("Parameter estimate in odd windows")))+
   theme(plot.title = element_text(hjust = 0.5, size=15)) # center the title
